...
import { Radar, Lock, User, Mail } from 'lucide-react';
...
export default function Register() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      navigate('/dashboard');
    } catch { setError('Erro ao criar conta. E-mail pode já estar em uso.'); } 
  ...
  return (
    <div className="min-h-screen bg-[#050912] flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-[#0b1628] border border-cyan-500/10 rounded-2xl p-8 space-y-5 shadow-2xl">
          <h2 className="text-white font-black text-sm uppercase tracking-widest mb-6">Criar Acesso</h2>

          {error && <div className="bg-red-500/10 border border-red-500/20 text-red-400 text-[10px] font-bold p-3 rounded-xl uppercase tracking-widest">{error}</div>}

          <div className="relative">
            <User className="absolute left-3 top-3.5 text-slate-600" size={16} />
            <input type="text" placeholder="Nome completo" value={name} onChange={(e) => setName(e.target.value)} className={inputClass} />
          </div>
          ...
          <div className="relative">
            ...
            <input type="password" placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} className={inputClass} />
          </div>

          <div className="relative">
            <Lock className="absolute left-3 top-3.5 text-slate-600" size={16} />
            <input type="password" placeholder="Confirmar Senha" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={inputClass} />
          </div>
          ...
          <button type="submit" disabled={loading} 
            ...
            {loading ? 'Inicializando...' : 'Criar Conta'}
          </button>

          <p className="text-[10px] text-slate-500 text-center uppercase tracking-widest mt-4">
            Já possui acesso?{' '}
            <Link to="/login" className="text-cyan-400 font-bold hover:underline">Entrar</Link>
          </p>
        </form>
      </div>
    </div>
